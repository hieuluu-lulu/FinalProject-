...
const ColorStrings = {
    RGB: {
        ...
        parse: function (result) {
            return {
                r: util.isPercentage(result[1])
                    ? util.conventPercentageToRgb(result[1])
                    : parseInt(result[1], 10),
                g: util.isPercentage(result[2])
                    ? util.conventPercentageToRgb(result[2])
                    : parseInt(result[2], 10),
                b: util.isPercentage(result[3])
        ...
        parse: function (result) {
            return {
                ...
                b: util.isPercentage(result[3])
                    ? util.conventPercentageToRgb(result[3])
                    : parseInt(result[3], 10),
                a: util.isPercentage(result[4])
                    ? util.convertPercentageToFloat(result[4])
    ...
    HSL: {
        ...
        parse: function (result) {
            const hsl = {
                ...
                s: util.isPercentage(result[2])
                    ? util.convertPercentageToFloat(result[2])
                    : parseFloat(result[2], 10),
                l: util.isPercentage(result[3])
                    ? util.convertPercentageToFloat(result[3])
    ...
    NAME: {
        ...
        to: function (color, instance) {
            ...
            return ColorStrings[
                instance.options.nameDegradation.toUpperCase()
            ].to(color);
        },
    },
};
