    ...
    mousedown: function (e) {
        ...
        this.mousemove = function (e) {
            ...
            if (this.direction === 'vertical') {
                position =
                    this.data.top +
                    (e.pageY || this.data.startY) -
                    this.data.startY;
            } else {
                position =
                    this.data.left +
                    (e.pageX || this.data.startX) -
                    this.data.startX;
            }
    ...
    init: function (api, options) {
        ...
        api.$element.on('asColorPicker::firstOpen', () => {
            if (that.options.apply) {
                that.$apply = $(
                    that.options.applyTemplate.call(that, api.namespace),
                )
...
var Gradient = function (api, options) {
    ...
    $.extend(that, {
        ...
        preview: {
            ...
            render() {
                that.$preview.css({
                    'background-image': that.value.toStringWithAngle(
                        'to right',
                        true,
                    ),
                });
            ...
            init() {
                ...
                that.$markers.on(that.api.eventName('mousedown'), (e) => {
                    ...
                    const position = parseFloat(
                        (e.pageX - that.$markers.offset().left) /
                            that.markers.width,
                        10,
                    );
                ...
                let self = this;
                that.$markers.on(
                    that.api.eventName('mousedown'),
                    'li',
                    function (e) {
                        const rightclick = e.which
                            ? e.which === 3
                            : e.button === 2;
                        if (rightclick) {
                            return false;
                ...
                that.$doc.on(that.api.eventName('keydown'), (e) => {
                    if (
                        that.api.opened &&
                        that.$markers.is(`.${that.classes.focus}`)
                    ) {
        ...
        wheel: {
            ...
            calAngle(x, y) {
                const deg = Math.round(
                    Math.atan(Math.abs(x / y)) * (180 / Math.PI),
                );
                if (x < 0 && y > 0) {
                    return 360 - deg;
                }
