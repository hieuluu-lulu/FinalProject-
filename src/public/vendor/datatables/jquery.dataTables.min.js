...
(function (h) {
    ...
    function wb(a) {
        ...
        for (g = 0; g < c; g++)
            (m = b[g]),
                ...
                u('mDataProp_' + g, i),
                d.bFilter &&
                    (u('sSearch_' + g, l.sSearch),
                    u('bRegex_' + g, l.bRegex),
                    u('bSearchable_' + g, m.bSearchable)),
                d.bSort && u('bSortable_' + g, m.bSortable);
            ...
            for (j = l.length; g < j; g++) {
                ...
                if (b === 'table') {
                    ...
                    for (h = o.length; i < h; i++) {
                        ...
                        f =
                            b === 'cell'
                                ? c.call(t, l[g], f.row, f.column, g, i)
                                : c.call(t, l[g], f, g, i, n);
                        f !== k && e.push(f);
                    }
                }
            }
    ...
    var bb = function (a, b, c, d, e) {
            var f = [],
                ...
                n;
            h = typeof b;
            if (!b || h === 'string' || h === 'function' || b.length === k)
                b = [b];
            h = 0;
            for (m = b.length; h < m; h++) {
                j =
                    b[h] && b[h].split && !b[h].match(/[\[\(:]/)
                        ? b[h].split(',')
                        : [b[h]];
                l = 0;
                for (n = j.length; l < n; l++)
                    (g = c(typeof j[l] === 'string' ? j[l].trim() : j[l])) &&
                        g.length &&
                        (f = f.concat(g));
            }
            a = v.selector[a];
